...
export const getServiceDesks = async (platform, configKey) => {
    const response = await api.get(`/${platform}/servicedesks`, { params: { configKey } });
    return response.data;
};

// Fetches request types for a specific service desk
export const getRequestTypes = async (platform, configKey, serviceDeskId) => {
    const response = await api.get(`/${platform}/servicedesks/${serviceDeskId}/requesttypes`, { params: { configKey } });
    return response.data;
};
...
export const getRequestTypeFields = async (platform, configKey, serviceDeskId, requestTypeId) => {
    const response = await api.get(`/${platform}/servicedesks/${serviceDeskId}/requesttypes/${requestTypeId}/fields`, { params: { configKey } });
    return response.data;
};
